            ...
            foreach ($budgets as $budget) {
                ...
                if ($percentage >= 100) {
                    $insights[] = [
                        ...
                        'message' => sprintf(
                            'Budget kategori "%s" sudah terlampaui %.0f%%. Total pengeluaran: Rp %s dari Rp %s.',
                    ...
                    $insights[] = [
                        ...
                        'message' => sprintf(
                            'Budget kategori "%s" sudah mencapai %.0f%%. Sisa: Rp %s untuk %d hari.',
                            $budget['category_name'],
                            $percentage,
    ...
    private function generateSpendingPatterns() {
        ...
        try {
            ...
            $query = "SELECT 
                      ...
                      COUNT(CASE WHEN DAYOFWEEK(transaction_date) IN (1, 7) THEN 1 END) as weekend_days,
                      COUNT(CASE WHEN DAYOFWEEK(transaction_date) NOT IN (1, 7) THEN 1 END) as weekday_days
                      FROM transactions
                      WHERE user_id = :uid
            ...
            if ($pattern['weekend_days'] > 0 && $pattern['weekday_days'] > 0) {
                $avgWeekend = $pattern['weekend_spending'] / $pattern['weekend_days'];
                $avgWeekday = $pattern['weekday_spending'] / $pattern['weekday_days'];
                ...
                if ($avgWeekend > $avgWeekday * 1.5) {
                    $insights[] = [
                        ...
                        'message' => sprintf(
                            'Rata-rata pengeluaran weekend (Rp %s) lebih tinggi %.0f%% dari weekday (Rp %s). Pertimbangkan aktivitas hemat di akhir pekan.',
        ...
        try {
            ...
            foreach ($topCategories as $category) {
                if ($category['grand_total'] > 0) {
                    ...
                    if ($percentage > 30) {
                        $potentialSavings = $category['total'] * 0.2; // Assume 20% reduction possible
                        $insights[] = [
                            'insight_type' => 'savings_opportunity',
                            'severity' => 'success',
                            'title' => 'Peluang Hemat: ' . $category['category_name'],
                            'message' => sprintf(
                                'Kategori "%s" menyerap %.0f%% dari total pengeluaran (Rp %s). Dengan mengurangi 20%%, Anda bisa hemat Rp %s per bulan.',
                ...
                if ($comp['last_month'] > 0) {
                    ...
                    if ($increase > 20) {
                        $insights[] = [
                            'insight_type' => 'spending_pattern',
                            'severity' => 'warning',
                            'title' => 'Lonjakan Pengeluaran: ' . $comp['category_name'],
                            'message' => sprintf(
                                'Pengeluaran kategori "%s" naik %.0f%% bulan ini (Rp %s) dibanding bulan lalu (Rp %s). Periksa transaksi tidak biasa.',
                                $comp['category_name'],
                                $increase,
